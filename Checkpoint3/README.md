# Checkpoint3 Instructions & Submission Requirements
...
🔥 You are not allowed to work from remote though browser. You must use local repo in your laptop and azure. You can use any **IDE** of your choice, though the preferred option in this course is Microsoft VS Code. Choose the tool you will use for your workflow management. You can use the GitHub Extension in VSCode or GitHub Desktop or simply the Git Bash CLI, which is always my preferred option.

🔥🔥 You can only do these steps once. If you try to repeat this step it will not produce same results! Maybe you would want to create a `Checkpoint-dummy` and experiment some of the steps before trying to do them in your `Checkpoint3` folder for submission.
...
🔥🔥🔥 Do **NOT** add you log files as you go, just do it after you finish all your work. The way this assignment works you will `reset` your commit `HEAD` a couple of times and your files will be deleted, so just add `logs` at the end before submission.
...
- In order to create a conflict, modify the file `./conflict-resolution/index.html` in VSCode (local repo). Edit same line and update the color to _**green**_
...
- **Create a `logs` folder OUTSIDE of your `CSP451-Azure-Project` folder in `<path-to-logs>/logs/`. After you complete all steps in this assignment, you will submit it. For now, just keep it outside of the repo. Follow later instructions for its submission**. I will use `<path-to-logs>/logs/` when using about.
...
- Do not miss the last step above, if you miss the last `git pull` you can complete Part C.
...
- ⚠️ **Start with creating a new branch in remote** called `feat-body-text`. Make sure you are working in your new branch. Serach the `./conflict-resolution/index.html` file, and find below line
...
- Start from VSCode (local repo) and make sure the working tree is clean. If you have `untracked files` or your `commit` is behind, make sure to do a the needful and clean your working tree. Run `git status` andd make sure the output looks like this:
...
- Check your remote in browser. After you have pushed your changes to remote, it should look somewhat like this:

<img src="./images/pull-request.jpg"
     alt="Pull Reuest"
     style="float: left; margin-right: 10px;" />
